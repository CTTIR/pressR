# exported API surface is unchanged
    ...
    Output
      ...
      pr_calc_dominant_freq(trial, band = c(0.3, 5), signal = "total", window = "hann")
      pr_calc_force(trial)
      pr_calc_gait_cycles(trial, force_threshold = 20, min_stance_ms = 200, min_swing_ms = 100)
      pr_calc_gradient(trial, statistic = "mean", method = "central", edges = c("na", "zero"), threshold = 0)
      ...
      pr_calc_phase_map(trial, cycles = NULL, n_bins = 10L, trim = 0.1, min_frames = 10L)
      pr_calc_pti(trial)
      pr_calc_regional(trial, masks = NULL, parameters = c("mpp", "mvp", "max_force", "contact_area", "pti_mean"), threshold = 0)
      pr_calc_regional_map(trial, masks = NULL, statistic = c("mean", "max", "loaded"), threshold = 0)
      ...
      pr_calc_spectrum(trial, signal = c("total", "mean", "peak"), window = c("hann", "none"), detrend = TRUE)
      pr_calc_stride_cycles(trial, signal = "total", lp_window = 11L, hp_window_s = 5, min_period_s = 0.3, height_quantile = 0.3, max_period_s = 3)
      pr_calc_symmetry_index(trial, parameter = c("peak_pressure", "mean_pressure", "force", "contact_area"))
      pr_calc_symmetry_map(trial, masks = NULL, statistic = "mean", denominator = c("grid", "loaded"))
      ...
      pr_mask_rowbands(layout, breaks = c(5, 11), sides = TRUE, band_names = c("cranial", "middle", "caudal"))
      ...
      pr_sensor_quality(dataset, dead_pct_zero = 95, saturation_kpa = NULL, low_var_sd = 0.1, threshold = 0)
      pr_summary(trial, threshold = 0)
      pr_symmetry_sensitivity(trial, masks, statistic = "mean", denominator = "grid", balance = TRUE)
